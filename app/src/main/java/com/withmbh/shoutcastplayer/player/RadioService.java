...
public class RadioService extends Service implements Player.EventListener, AudioManager.OnAudioFocusChangeListener, ShoutcastMetadataListener {
    ...
    public static final String ACTION_PLAY = "com.withmbh.shoutcastplayer.player.ACTION_PLAY";
    public static final String ACTION_PAUSE = "com.withmbh.shoutcastplayer.player.ACTION_PAUSE";
    public static final String ACTION_STOP = "com.withmbh.shoutcastplayer.player.ACTION_STOP";
    ...
    private final okhttp3.Call.Factory factory = new OkHttpClient.Builder().build();

    private final static String TAG = RadioService.class.getName();
...
//        // https://github.com/KimKJ/ShoutcastSample
//        DefaultBandwidthMeter bandwidthMeter = new DefaultBandwidthMeter();
//        DataSource.Factory dataSrcFactory = new IcyDataSourceFactory(Util.getUserAgent(getApplicationContext(), this.getString(R.string.app_name)), null);
//        DefaultExtractorsFactory extrFactory = new DefaultExtractorsFactory();
//        ExtractorMediaSource mediaSource = new ExtractorMediaSource(Uri.parse(streamUrl), dataSrcFactory, extrFactory, null, null);

//        //
//        DataSource.Factory dataSourceFactory =  new ShoutcastDataSourceFactory(factory, Util.getUserAgent(this, this.getString(R.string.app_name)), null, RadioService.this);
//        DefaultExtractorsFactory extrFactory = new DefaultExtractorsFactory();
//        ExtractorMediaSource mediaSource = new ExtractorMediaSource(Uri.parse(streamUrl), dataSourceFactory, extrFactory, null, null);
        ...
        IcyHttpDataSourceFactory factory = new IcyHttpDataSourceFactory.Builder(Util.getUserAgent(this, getResources().getString(R.string.app_name)))
                .setIcyHeadersListener(
                        icyHeaders -> {
                            Log.i(TAG, "metadata : " + icyHeaders.toString());
                        }
                )
                .setIcyMetadataChangeListener(
                        icyMetadata  -> {
                            Log.i(TAG, "metadata : " + icyMetadata.toString());
                        }
                ).build();
        DefaultDataSourceFactory dataSourceFactory = new DefaultDataSourceFactory(getApplicationContext(), null, factory);
        DefaultExtractorsFactory extrFactory = new DefaultExtractorsFactory();
        ExtractorMediaSource mediaSource = new ExtractorMediaSource(Uri.parse(streamUrl), dataSourceFactory, extrFactory, handler, null);
    ...
    class MetadataTask extends AsyncTask<Void,Void,Void>
    {
        ...
        protected Void doInBackground(Void... params) {
            URL url = null;
            try {
                url = new URL("http://god.inlive.co.kr:2848/");
            } catch (MalformedURLException e) {
                e.printStackTrace();
            }
            streamMeta = new IcyStreamMeta(url);
            try {
